...
@Controller
@Scope(value = "view")
public class OrdenPreparacionMezclaMB implements Serializable {

    private static final long serialVersionUID = 1L;
    private static final Logger LOGGER = LoggerFactory.getLogger(OrdenPreparacionMezclaMB.class);
...
//    private transient TipoSolucionService tiposolucionService;
    ...
    @PostConstruct
    public void init() {
        LOGGER.trace("mx.mc.magedbean.OrdenPreparacionMezclaMB.init()");
        ...
        permiso = Comunes.obtenerPermisos(Transaccion_Enum.ORDNUTRIPARENTERAL.getSufijo());
        ...
        try {
            ...
            obtenerEnvases();
// TODO: 15/FEB/2024 HHRC Generar información en tabla de BD
            idTipoSolucion = "e695144c-0a63-11eb-a03a-000c29750049";
...
// TODO: 07/ABR/2024 HHRC Generar información en tabla de BD
    ...
    public void obtenerCamas() {
        LOGGER.trace("mx.mc.magedbean.OrdenPreparacionMezclaMB.obtenerCamas()");
    ...
    public void obtenerTipoPacientes() {
        LOGGER.trace("mx.mc.magedbean.OrdenPreparacionMezclaMB.obtenerTipoPacientes()");
    ...
    public void initialize() {
        LOGGER.trace("mx.mc.magedbean.OrdenPreparacionMezclaMB.initialize()");
        paramError = "error";
...
// TODO: 15/FEB/2024 HHRC Generar información en tabla de BD            
            String idEstructuraNPT = "e39332fa-9344-49e7-b588-6a3353f86bd6";
            this.estructuraList.add(estructuraService.obtenerEstructura(idEstructuraNPT));
            idEstructura = this.estructuraList.get(0).getIdEstructura();
        } catch (Exception e) {
            LOGGER.error("Error en el metodo alimentarComboAlmacen :: alimentarComboAlmacen{}", e.getMessage());
        }
    }
    ...
    private List<Estructura> obtenerEstructuras() {
        ...
        try {
            ...
            LOGGER.error("Error en el metodo obtenerEstructuras :: {}", e.getMessage());
    ...
    public void nuevaMezcla() {
        LOGGER.trace("mx.mc.magedbean.OrdenPreparacionMezclaMB.nuevaMezcla()");
        boolean res = false;
        if (usuarioSelected.getIdUsuario() == null) {
            ...
            try {
                ...
                nutricionParenteralSelect.setIdEstatusSolucion(EstatusSolucion_Enum.PRESCRIPCION_REGISTRADA.getValue());
    ...
    public void buscarOrdenesPreparacion() {
        LOGGER.trace("mx.mc.magedbean.OrdenPreparacionMezclaMB.buscarOrdenesPreparacion()");
        try {
            //Se buscan las ordenes de preparación
            List<Integer> estatusSolucionList = new ArrayList<>();
            estatusSolucionList.add(EstatusSolucion_Enum.PRESCRIPCION_SOLICITADA.getValue());
     ...
     *
     * @param idNutricionParenteral
     */
    public void obtenerDetalleOrdenPreparacion(String idNutricionParenteral) {
        this.validaFormulario = "";
        ...
        if (nutricionParenteralSelect == null) {
            ...
            if (permiso.isPuedeAutorizar()) {
                this.mostrarBotones = Constantes.ACTIVO;
            } else if (Objects.equals(nutricionParenteralSelect.getIdEstatusSolucion(), EstatusSolucion_Enum.PRESCRIPCION_SOLICITADA.getValue())
...
//Se busca la nutricion parenteral que se selecciono
                ...
                if (nutricionParenteralSelect == null) {
                    ...
                    if (nutricionParenteralSelect.getIdPaciente() != null) {
                        ...
                        visitaAbierta = solUtils.recuperaVisita(nutricionParenteralSelect.getIdPaciente(), usuarioSelected.getIdUsuario());
                        if (visitaAbierta == null) {
                            ...
                            if (nutricionParenteralSelect.getIdMedico() != null) {
                                ...
                                pacienteServ = solUtils.recuperaPacienteServicio(visitaAbierta.getIdVisita(), nutricionParenteralSelect.getIdEstructura(), usuarioSelected.getIdUsuario(), nutricionParenteralSelect.getIdMedico(), nutricionParenteralSelect.getIdPaciente());
                                if (pacienteServ == null) {
                                    Mensaje.showMessage(Constantes.MENSAJE_ERROR, "Error al asignar servicio.", null);

                                } else {
                                    PacienteUbicacion pacUbi = solUtils.recuperaPacienteUbicacion(pacienteServ.getIdPacienteServicio(), usuarioSelected.getIdUsuario(), nutricionParenteralSelect.getIdCama(), nutricionParenteralSelect.getIdPaciente());
                                    if (pacUbi != null) {
                                        String idPacienteUbicacion = pacUbi.getIdPacienteUbicacion();
                                    }
                                }
                            }
                        }
                        ...
                        for (NutricionParenteralDetalleExtended nutricionGuardada : listaMedicamentosGuardados) {
                            for (NutricionParenteralDetalleExtended nutricionParenteral : nutricionParenteralSelect.getListaMezclaMedicamentos()) {
                                if (nutricionParenteral.getIdMedicamento().equals(nutricionGuardada.getIdMedicamento())) {
                                    ...
                                    nutricionParenteral.setSobrellenado(nutricionGuardada.getSobrellenado());
                                    break;
                                }
                            }
                        }
...
// TODO: 17/AGO/2023 HHRC Generar información en tabla de BD
                    idAlmacen = "a79709db-128b-4a91-a85f-81cb5cd37774";
                    ...
                    for (NutricionParenteralDetalleExtended nutricionPar : nutricionParenteralSelect.getListaMezclaMedicamentos()) {
                        if (nutricionPar.getVolPrescrito().compareTo(BigDecimal.ZERO) == 1) {
                            List<InventarioExtended> listInventario = inventarioService.obtenerInventarioPorIdEstructurasIdInsumo(idEstructuraList, nutricionPar.getIdMedicamento());
                            if (listInventario != null) {
                                if (!listInventario.isEmpty()) {
                                    nutricionPar.setListaInventarios(listInventario);
                                }
                    ...
                    evaluaEdicion();
                    nutricionParenteralSelect.setProteccionLuz(true);
                    nutricionParenteralSelect.setTempRefrigeracion(true);
                    nutricionParenteralSelect.setNoAgitar(true);
                }
            } catch (Exception ex) {
                LOGGER.error("Error al buscar el detalle de la orden de preparación :: {} ", ex.getMessage());
            }
            generaInstruccionesPreparacion();
        }
    }
...
// TODO: 03/MAR/2024 HHRC Los insumos de NTP dificilmete cambian en sus b¿valores de osmoloradidad y densidad, se opta por manjarse a nivel catálogo
...
// RN: si el insumo tiene osmolaridad, se calcula y se suma la Osmolaridad        
    ...
    * Método que realiza los cálculos de Osmolaridad total, NCA, estabilida, compatibilidad
    ...
    private void calculosDeOrdenDePreparacion(
        ...
        LOGGER.trace("mx.mc.magedbean.OrdenPreparacionMezclaMB.calculosDeOrdenDePreparacion()");
        int cont = 1;
...
//            nutricionParenteralSelect.setResultEstabilidad("INESTABLE");
    ...
    public void onCellEdita(CellEditEvent event) {
        try {
            ...
            if (actualizaMedica) {
                ...
                for (NutricionParenteralDetalleExtended nutricion : nutricionParenteralSelect.getListaMezclaMedicamentos()) {
                    BigDecimal calorias = BigDecimal.ZERO;
                    // TODO Falta revisar como obtener este valor devido a que se multiplica por diferente valor en algunos insumos
                    if (nutricion.isCalculoMezcla()) {
                        ...
                        if (mediCaloria != null) {
                            ...
                            if (nutricion.getClaveMedicamento().equals("010.000.0006.00")) {
                                if (nutricion.getVolPrescrito() != null) {
                                    ...
                                    nutricion.setPorcentajeCalculado(nutricion.getVolPrescrito().multiply(mediCaloria.getConcentracion()));
                                }
                            } else {
            ...
            LOGGER.error("Ocurrio un error al actualiza la concentración de medicamento" + ex.getMessage());
        }
    }
    ...
    public void selectPaciente(SelectEvent e) {
        LOGGER.trace("mx.mc.magedbean.OrdenPreparacionMezclaMB.selectPaciente()");
...
// TODO: cosultar información de base de datos
                    if (pacienteSelect.getEdadPaciente() != null) {
                        if (pacienteSelect.getEdadPaciente() >= 0 && pacienteSelect.getEdadPaciente() < 1) {
                            ...
                            nutricionParenteralSelect.setSobrellenado(20);
                        } else if (pacienteSelect.getEdadPaciente() >= 9) {
     ...
     * para generar la orden de preparación
     */
    ...
    public void agregarPaciente() {
        LOGGER.trace("mx.mc.magedbean.OrdenPreparacionMezclaMB.agregarPaciente()");
    ...
    private String validarClavePaciente() {
        ...
        try {
            Paciente pacienteResp = pacienteService.obtener(pacienteTem);
    ...
    public void agregarMedicos() {
        ...
        if (this.medicoExtended != null) {
            ...
            try {
                estatus = usuarioService.insertarMedicoYTurno(medicoTemp, listaTemp);
            } catch (Exception ex) {
                estatus = false;
                Mensaje.showMessage(Constantes.MENSAJE_ERROR, RESOURCES.getString("prc.manual.error.registro"), "");
    ...
    /**
     * Valida la orden de preparación de NTP
     */
    ...
    public void guardarOrdenPreparacion(Integer idEstatusNutricion) {
        ...
        try {
            estatus = validaGuardaOrdenPreparacion(idEstatusNutricion);
            if (estatus){
                buscarOrdenesPreparacion();
            }
        } catch (Exception ex) {
    ...
    private boolean validaGuardaOrdenPreparacion(Integer idEstatusNutricion) {
        LOGGER.trace("mx.mc.magedbean.OrdenPreparacionMezclaMB.validaGuardaOrdenPreparacion()");
        boolean estatus = Constantes.INACTIVO;
        try {
            if (usuarioSelected == null) {
                Mensaje.showMessage(Constantes.MENSAJE_ERROR, "Usuario inválido ", null);
...
//                        }
                    ...
                    SurtimientoEnviado surtimientoEnviado;
                    Integer cuenta = 1;
                    Integer horas;
                    BigDecimal mls;
                    ...
                    LOGGER.trace("solUtils.creaSolucion");
                    Solucion solucion = solUtils.creaSolucion(idSurtimiento, nutricionParenteralSelect, usuarioSelected);
                    ...
                    if (editar) {
                        ...
                        Solucion sol = solUtils.obtenerSolucion(idSolucion, idSurtimiento);
                        if (sol != null) {
                            solucion.setIdSolucion(sol.getIdSolucion());
            ...
            LOGGER.error("Error al validar y guardar la orden de preparación :: {} ", e.getMessage());
        }
        return estatus;
    }

    public void validarOrdenPreparacion() {
        LOGGER.trace("mx.mc.magedbean.OrdenPreparacionMezclaMB.validarOrdenPreparacion()");
        boolean estatus = Constantes.INACTIVO;
        try {
            boolean result = validaGuardaOrdenPreparacion(EstatusSolucion_Enum.PRESCRIPCION_VALIDADA.getValue());
            
            if (result && validaFormulario.isEmpty()) {
                ...
                if (nutricionParenteralSelect.getDiagnosticos() != null) {
                    if (!nutricionParenteralSelect.getDiagnosticos().isEmpty()) {
                        ...
                        if (lisDiagnosticos.length > 0) {
                            for (int i = 0; i < lisDiagnosticos.length; i++) {
                                if (!"".equals(lisDiagnosticos[i].toString())) {
...
//Generar la visita nueva del paciente                
                    if (nutricionParenteralSelect.getNumeroVisita() == null
                            && !nutricionParenteralSelect.getNumeroVisita().isEmpty()) {
                        ...
                        estatus = nutricionParenteralService.generarNuevaVisitaYPacienteServicio(visitaAbierta, pacienteServ);
                        if (estatus) {
                            verificarUbicacion();
                        }
                    }
    ...
    public void continuarValidacionOrdenPreparada() {
        try {
            ...
            if (prescripcionPaciente != null) {
                ...
                prescripcion.setIdTipoSolucion(idTipoSolucion);
            }
...
// TODO: HHRC Valores genéricos de NTP
...
//                listaPrescripcionInsumos.get(0).setRitmo(0.033d);
     ...
     * @param item
     */
    public void previewPrint(NutricionParenteralExtended item) throws Exception {
...
//                    if (nutricionParenteralSelect.getInstruccionPreparacion() == null
//                            || nutricionParenteralSelect.getInstruccionPreparacion().trim().isEmpty()){
                        List<NutricionParenteralDetalleExtended> listaMedicamentosParenterales = medicamentoService.obtenerMedicamentosParentales();
                        nutricionParenteralSelect.setListaMezclaMedicamentos(listaMedicamentosParenterales);
                        StringBuilder instrucionPreparacion = solUtils.generarInstrucPreparaNPT(nutricionParenteralSelect);
                        String text =  instrucionPreparacion.toString();
//                        text = "Del contenedor Bolsa Eva Agregar: \n" +
//"    * AMINOACIDOS CRISTALINOS AL 8% ENRIQUECIDOS CON AMINOACIDOS DE CADENA RAMIFICADA 42% 130.9000 mL\n" +
//"    * AMINOACIDOS CRISTALINOS PEDIATRICOS AL 10% 94.2000 mL\n" +
//"    * AMINOACIDOS CRISTALINOS AL 8.5% CON ELECTROLITOS 98.7180 mL\n" +
//"    * AMINOACIDOS CRISTALINOS DE ADULTO AL 10% 73.5000 mL\n" +
//"    * FOSFATO DE POTASIO SOLUCIÓN INYECTABLE 3.2000 mL\n" +
//"    * CLORURO DE SODIO AL 17.7% 1.8670 mL\n" +
//"    * CLORURO DE POTASIO SOLUCIÓN INYECTABLE 1.3000 mL\n" +
//"    * CLORURO DE POTASIO SOLUCIÓN INYECTABLE 1.3000 mL\n" +
//"    * L-GLUTAMINA SOLUCIÓN INYECTABLE 10.3000 mL\n" +
//"    * LEVOCARNITINA 52.2000 mL\n" +
//"    * GLUCOSA. SOLUCIÓN INYECTABLE AL 50 % , ENVASE 50ML 18.7000 mL\n" +
//"    * GLUCOSA. SOLUCIÓN INYECTABLE AL 50 %, ENVASE 250 ML 16.8000 mL\n" +
//"    * Agua Inyectable. Envase con 500 ml. 6.39 mL\n" +
//"    * SULFATO DE MAGNESIO SOLUCIÓN INYECTABLE 7.7070 mL\n" +
//"    * ACIDO ASCORBICO SOLUCIÓN INYECTABLE  (VITAMINA C) -0.0500 mL\n" +
//"    * OLIGOMETALES ENDOVENOSOS SOLUCIÓN INYECTABLE 4.5000 mL\n" +
//"    * ZINC SOLUCIÓN INYECTABLE 3.2000 mL\n" +
//"    * CROMO SOLUCIÓN INYECTABLE 0.6000 mL\n" +
//"    * GLUCONATO DE CALCIO  SOLUCIÓN INYECTABLE 2.6220 mL\n" +
//"    * LIPIDOS AL 20% MCT/LCT,  ACEITE DE SOYA 52.2000 mL\n" +
//"    * LIPIDOS AL 20%  MCT/LCT , ACEITE DE SOYA, ACEITE DE OLIVA,  TRIGLICERIDOS CM Y ACEITE DE PESCADO RICO EN ACIDOS OMEGA 3. (SMOFLIPID) 47.1000 mL\n" +
//"    * ACIDOS GRASOS OMEGA 3 1.9000 mL\n" +
//"    * MULTIVITAMINICO ADULTO 7.1000 mL\n" +
//"    * MULTIVITAMINICO PEDIATRICO 6.4000 mL\n" +
//"    * Ácido FOLÍnico 50 mg 0.6000 mL\n" +
//"    * FITOMENADIONA SOLUCIÓN INYECTABLE (VITAMINA K) 0.6000 mL\n" +
//"    * HEPARINA SOLUCIÓN INYECTABLE 1000 UI/ml 0.0030 mL\n" +
//"    * INSULINA HUMANA DE ACCIÓN RAPIDA 0.0100 mL\n" +
//"    * Agua Inyectable. Envase con 500 ml. 0.7100 mLmL";
                        nutricionParenteralSelect.setInstruccionPreparacion(text);
//                    }
    ...
    public void seleccionaCama() {
        LOGGER.trace("mx.mc.magedbean.OrdenPreparacionMezclaMB.seleccionaCama()");
        try {
    ...
    private void obtieneServiciosQuePuedeSurtir() {
        LOGGER.trace("mx.mc.magedbean.OrdenPreparacionMezclaMB.obtieneServiciosQuePuedeSurtir()");
    ...
    public void generaInstruccionesCondiciones() {
        LOGGER.trace("mx.mc.magedbean.OrdenPreparacionMezclaMB.generaInstruccionesCondiciones()");
...
//                        Double sumaVolPorDrenar = this.surtimientoExtendedSelected.getVolumenTotal() - vol
    ...
    public void generaInstruccionesPreparacion() {
        LOGGER.trace("mx.mc.magedbean.OrdenPreparacionMezclaMB.generaInstruccionesPreparacion()");
        try {
            // 1.- recorrer la lista de NPT prescrito
            // 2.- buscar el medicamento prescrito con dosis > 0
            // 3.- Generar un texto de la siguiente manera
            //       Al contenedor #CONTENEDOR#
            //       Agregar #CANTIDAD_PRESCRITA# #UNIDAD_CONCENTRACION#  #VOLUMEN_CALCULADO# mL
            //       RECORRER TODA LA LISTA CON EL ORDEN INDICADO EN EL SIGUIENTE ARRAY
            //       PARA EL CASO DEL AGUA INYECTABLE OBTENER EN 1 PRIMER PASO EL 90% Y AL FINAL EL 10%
//            List<String> listaInsumoNpt = new ArrayList<>();
//            listaInsumoNpt.add("010.000.2512.01");
//            listaInsumoNpt.add("010.000.2512.00");
//            listaInsumoNpt.add("010.000.2738.00");
//            listaInsumoNpt.add("010.000.0010.00");
//            listaInsumoNpt.add("010.000.0007.00");
//            listaInsumoNpt.add("010.000.0003.00");
//            listaInsumoNpt.add("010.000.0015.00");
//            listaInsumoNpt.add("010.000.0004.00");
//            listaInsumoNpt.add("010.000.0017.00");
//            listaInsumoNpt.add("010.000.3629.00");
//            listaInsumoNpt.add("010.000.0012.00");
//            listaInsumoNpt.add("010.000.0011.00");
//            listaInsumoNpt.add("010.000.5379.00");
//            listaInsumoNpt.add("010.000.0025.00");
//            listaInsumoNpt.add("010.000.5377.00");
//            listaInsumoNpt.add("010.000.5377.01");
//            listaInsumoNpt.add("010.000.0009.00");
//            listaInsumoNpt.add("010.000.0006.00");
//            listaInsumoNpt.add("010.000.0023.00");
//            listaInsumoNpt.add("010.000.0014.00");
//            listaInsumoNpt.add("010.000.5384.00");
//            listaInsumoNpt.add("010.000.5385.00");
//            listaInsumoNpt.add("010.000.5385.01");
//            listaInsumoNpt.add("010.000.1707.00");
//            listaInsumoNpt.add("010.000.2152.00");
//            listaInsumoNpt.add("010.000.2192.00");
            
            StringBuilder instrucionPreparacion = solUtils.generarInstrucPreparaNPT(nutricionParenteralSelect);
            nutricionParenteralSelect.setInstruccionPreparacion(instrucionPreparacion.toString());

        } catch (Exception e) {
            LOGGER.error("Error al generar instruccioes de preparacion. :: {} ", e.getMessage());
        }
    }
...
//                }
    ...
    /**
     * Realiza la validación del rechazo de la Mezcla
     * 
     */
    public void rechazarValidacionSolucion() {
        LOGGER.trace("mx.mc.magedbean.OrdenPreparacionMezclaMB.rechazarValidacionSolucion()");
        boolean estatus = Constantes.INACTIVO;
        try {
            if (this.usuarioSelected == null) {
                Mensaje.showMessage(Constantes.MENSAJE_ERROR, "Usuario inválido.", null);
...
//TODO: 23/JUL/2024 HHRC rechazar vaidación
                Surtimiento surtimiento = surtimientoService.obtenerSurtimientoExtendedByIdSurtimiento(nutricionParenteralSelect.getIdSurtimiento());
                Solucion o = solucionService.obtenerSolucion(null, nutricionParenteralSelect.getIdSurtimiento());
                if (o == null) {
                    Mensaje.showMessage(Constantes.MENSAJE_ERROR, "Solución inválida.", null);
                    ...
                    List<SurtimientoInsumo_Extend> surtimientoInsumoExtendedList = surtimientoInsumoService.obtenerListaByIdSurtimiento(nutricionParenteralSelect.getIdSurtimiento());

                    estatus = solucionService.rechazarMezcla(s, surtimiento, surtimientoInsumoExtendedList, false);
                }
                ...
                if (!estatus) {
                    ...
                    if (this.nutricionParenteralSelect != null) {
                        Paciente pac = pacienteService.obtenerPacienteByIdPaciente(this.nutricionParenteralSelect.getIdPaciente());
                        if (pac != null) {
                    ...
                    this.nutricionParenteralSelect.setNombrePaciente(nombrePaciente.toString());
                    this.nutricionParenteralSelect.setPacienteNumero(pacienteNumero);
                    
                    Surtimiento_Extend surtimientoExtended = new Surtimiento_Extend();
                    surtimientoExtended.setIdPrescripcion(surtimiento.getIdPrescripcion());
                    
                    Prescripcion p = this.solUtils.obtenerPrescripcion(surtimientoExtended);
                    if (p != null) {
                        String IdUsuario = p.getIdMedico();
                        Usuario u = this.solUtils.obtenerUsuarioPorId(IdUsuario);
                        if (u != null) {
                            String asunto = "Prescripción de mezcla " + surtimientoExtended.getFolioPrescripcion() + " Rechazada. ";
                            ...
                            this.solUtils.enviarCorreo(u, this.usuarioSelected, asunto, msj.toString(), surtimientoExtended);
                        }
                    }
                }
    ...
    /**
     * Realiza las acciones de validación y para rechazo de orden de preparación prescripción
     * 
     */
    public void validaRechazo() {
        LOGGER.trace("mx.mc.magedbean.OrdenPreparacionMezclaMB.validaRechazo()");
        boolean estatus = false;
        this.solucion = new Solucion();
